...
def bfs(graph, start_node):
    ...
    while queue:    # 큐에 노드가 있는 동안
        ...
        for station in current_station.adjacent_stations:   # 인접한 노드를 돌면서
            if station.visited == False:    # 방문하지 않은 노드면
                station.visited = True  # 방문 표시를 하고
                queue.append(station)   # 큐에 넣는다
